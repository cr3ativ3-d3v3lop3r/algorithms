...
object ReduceLeft extends App {
  ...
  def sum(list: List[Int]): Int = list.reduceLeft {
    (left, right) => left + right
  }

  def product(list: List[Int]): Int = list.reduceLeft {
    (left, right) => left * right
  }

  def min(list: List[Int]): Int = list.reduceLeft {
    (left, right) => left min right
  }

  def max(list: List[Int]): Int = list.reduceLeft {
    (left, right) => left max right
  }

  def largest(list: Vector[String]): String = list.reduceLeft {
    (left, right) => if (left.length > right.length) left else right
  }

  def shortest(list: Vector[String]): String = list.reduceLeft {
